...
#Longitude is the first coordinate and lattitude is the second.
...
#The box in the top right corner changes from showing an illustration of layers, to showing the actual 3 layers which are geo, aerical and psysical.  
...
saveWidget(DANmap2, "DANmap2.html", selfcontained = TRUE)
...
places[69,] # Bønnerup strand
places[70,] # Hacky Hours
places[71,] # Tunø
...
# Clustering the plots can give a different overview of the data and show the data in a more readable way. 
...
DANmap_display <- DANmap %>%
  ...
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Placename,"<br>",places$Description, "<br>", places$Type, "<br>", places$Notes),
             clusterOptions = markerClusterOptions()) %>% 
  addMeasure(
    position = "bottomleft",
    primaryLengthUnit = "meters",
